      ...
      value: function _updateAddToCartButton(newVariant) {
        ...
        if (!newVariant) {
          ...
          if (newVariant['available']) {
            ...
            // LT 2022 
            // Custom ATC Text on PDP for products that are marked unavailable during pre-order phase
            // ALL 'PAY IN FULL' PRODUCT VARIANT OPTIONS WILL BE MARKED SOLD OUT 
            // if product tags include 'coming-soon' (see product-template.liquid) / has class 'pre-order-product'

            let productFormText = window.languages.productFormSoldOut; // LT
            let info_wrapper = $(".Product__InfoWrapper")[0]
            if(info_wrapper.classList.contains('pre-order-product')){
              // console.log('has pre-order item')
              // console.log('is not available') // LT
              let options = $('.ProductForm__OptionName')
              // console.log('options are', options.length )// LT
              for(let i = 0; i < options.length; i ++ ){
                if(options[i].innerText.includes('pre-order')){
                  let preorder_option = options[i].innerText
                  // console.log('preorder_option', preorder_option)// LT
                  if(preorder_option.includes('pay in full')){

                    // console.log('is a pre-order, variant is pay in full ')// LT
                    productFormText = 'COMING SOON'
                  }
                }
              }
            }
            ...
            addToCartButton.classList.add('Button--secondary');
            addToCartButton.removeAttribute('data-action');
            addToCartButton.innerHTML = productFormText // LT
          }
        }
